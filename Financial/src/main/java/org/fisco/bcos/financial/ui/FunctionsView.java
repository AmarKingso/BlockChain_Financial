...
public class FunctionsView extends JFrame implements ActionListener{
	 ...
	 * Create the frame.
	 */
	public FunctionsView(FinancialClient c, String title, boolean top, boolean down) {
		client = c;
	...
	public void actionPerformed(ActionEvent arg0) {
		if(arg0.getActionCommand() == "返回") {
			...
			lblMsg.setText("交易处理中...");
			if(lblTitle.getText() == "签订单据") {
				client.transactionFinancial(textField.getText(), new BigInteger(textField1.getText()));
			}
			else if(lblTitle.getText() == "转让单据") {
				client.transferFinancial(textField.getText(), new BigInteger(textField1.getText()));
			}
			else if(lblTitle.getText() == "申请融资") {
				client.financingFinancial(new BigInteger(textField1.getText()));
			}
			else {
				client.settlementFinancial(textField.getText());
			}
			lblMsg.setText("交易处理完成!");
		}
	}
}
